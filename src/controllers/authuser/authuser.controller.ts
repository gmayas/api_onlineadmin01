...
export const signUp = async (req: Request, res: Response): Promise<Response> => {
    try{
        ...
        emailuserExists = await pool.query('SELECT id FROM authuser.users WHERE emailuser = $1', [newUser.emailuser]);
...
export const profile = async (req: Request, res: Response): Promise<Response> => {
    try{
        const response: QueryResult = await pool.query('SELECT * FROM authuser.users WHERE emailuser = $1 ORDER BY id ASC LIMIT 1', [req.emailUser]);
...
export const updatePasswordUser = async (req: Request, res: Response): Promise<Response> => {
  try{
        ...
        console.log('newUser: ', newUser)
        // update new password
        const response = await pool.query('UPDATE authuser.users SET passworduser = $1 WHERE emailuser = $2', [newUser.passworduser, newUser.emailuser]);
        return res.status(200).json({ 
...
export const getTypeOfUser = async (req: Request, res: Response): Promise<Response> => {
    try{
        const response: QueryResult = await pool.query('SELECT * FROM authuser.typeofuser Order by usertype;');
        return res.status(200).json({
            message: 'Query succesfully',
            data: response.rows
        });   
